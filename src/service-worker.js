...
// ======================================================================
// 2. ÉP BUỘC CẬP NHẬT PWA (Fix lỗi kẹt phiên bản cũ không chịu reload)
// ======================================================================
self.addEventListener('install', function () {
    // Ép Service Worker mới tải về cài đặt ngay lập tức
    self.skipWaiting(); 
});

self.addEventListener('activate', function (event) {
    // Ép Service Worker mới giành quyền điều khiển trang web ngay lập tức
    ...
    try {
        ...
        event.waitUntil(
            self.registration.showNotification(title, options)
                .then(() => console.log('[Service Worker] ✅ Đã hiển thị popup thông báo ra màn hình thành công!'))
                .catch(err => console.error('[Service Worker] ❌ Lỗi khi vẽ popup:', err))
        );
