...
export default function TenantAddMemberModal({ open, onClose, onSuccess }) {
    const initialState = {
        full_name: "", phone: "", email: "", id_card_number: "", relationship: "friend", move_in_date: new Date().toISOString().slice(0, 10), note: ""
    };
    ...
    const handleImageUpload = async (e, type) => {
        ...
        if (type === 'front') {
            setFrontImg(Object.assign(file, { preview: URL.createObjectURL(file) }));
            // Gọi AI quét CCCD
            setIsScanning(true);
            try {
                ...
                if (full_name || id_card_number) {
                    ...
                    toast.success("Đã trích xuất CCCD thành công!");
                }
            } catch (error) {
                toast.warning("Ảnh mờ, vui lòng nhập tay thông tin.");
            } finally {
                setIsScanning(false);
            }
        } else {
            setBackImg(Object.assign(file, { preview: URL.createObjectURL(file) }));
        }
        e.target.value = "";
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (frontImg) data.append("id_card_front_image", frontImg);
            if (backImg) data.append("id_card_back_image", backImg);

            await tenantMemberService.create(data);
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-2xl max-h-[90vh] rounded-2xl flex flex-col shadow-2xl overflow-hidden">
                <div className="px-5 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <h2 className="text-[17px] font-bold text-slate-800">Thêm người ở chung</h2>
                    ...
                    <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="relative border-2 border-dashed border-slate-300 rounded-xl bg-slate-50 flex flex-col items-center justify-center aspect-[1.6/1] overflow-hidden hover:border-brand transition-colors group">
                            {frontImg ? <img src={frontImg.preview} className="w-full h-full object-cover" /> : (
                                <div className="text-center p-4">
                                    ...
                                    <p className="text-[11px] font-medium text-slate-500">Mặt trước CCCD (Tự động điền)</p>
                                </div>
                            )}
                        ...
                        <div className="relative border-2 border-dashed border-slate-300 rounded-xl bg-slate-50 flex flex-col items-center justify-center aspect-[1.6/1] overflow-hidden hover:border-brand transition-colors group">
                            {backImg ? <img src={backImg.preview} className="w-full h-full object-cover" /> : (
                                <div className="text-center p-4">
                                    <i className="fa-solid fa-camera text-2xl text-slate-300 mb-2"></i>
                                    <p className="text-[11px] font-medium text-slate-500">Mặt sau CCCD (Tùy chọn)</p>
                                </div>
                            )}
                ...
                <div className="px-5 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                    ...
                    <button onClick={handleSubmit} disabled={isSubmitting} className="px-6 py-2 bg-brand text-white rounded-lg text-[13px] font-bold shadow-sm flex items-center gap-2">
                        {isSubmitting && <i className="fa-solid fa-spinner fa-spin"></i>} Lưu lại
                    </button>
                </div>
            </div>
        </div>
