...
// Nhớ đổi lại tên service nếu bạn dùng file service riêng cho tenant nhé
...
export default function TenantViewIncidentModal({
    open,
    onClose,
    ...
    // State quản lý việc Zoom ảnh giống phần điện nước
    ...
    useEffect(() => {
        if (open && incident) {
            ...
            setIsLoading(true);
            tenantIncidentService.getById(incident.id)
                .then(res => {
    ...
    // Render danh sách ảnh (Có tích hợp Zoom)
                ...
                <div className="flex flex-wrap gap-2.5 sm:gap-3">
                    {images.map((img) => (
                        <div 
                            ...
                            className="relative w-[70px] h-[70px] sm:w-20 sm:h-20 rounded-lg border border-slate-200 overflow-hidden group block shadow-sm cursor-pointer"
                        >
        ...
        <>
            ...
            <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center bg-slate-900/60 backdrop-blur-sm sm:p-4 transition-all">
                <div className="bg-white w-full sm:max-w-2xl max-h-[90vh] sm:max-h-[85vh] rounded-t-2xl sm:rounded-2xl flex flex-col shadow-2xl overflow-hidden animate-slide-up sm:animate-fade-in relative">
                    ...
                    {/* Body */}
                    <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] p-4 sm:p-5 bg-white">
                        <div className="space-y-5 sm:space-y-6">
                            ...
                            {/* 3. Dữ liệu tài chính & Kết quả xử lý (Giao diện Khách thuê) */}
                                    ...
                                    {data.status === 'cancelled' ? (
                                        ...
                                        <div className="flex flex-col gap-2">
                                            <div className="flex justify-between items-center text-[13px] sm:text-[14px]">
                                                <span className="text-slate-600">Tổng chi phí sửa chữa:</span>
                                            ...
                                            <div className="flex justify-between items-center text-[12px] sm:text-[13px]">
                                                <span className="text-slate-500">Trách nhiệm thanh toán:</span>
                                                <span className={`font-semibold ${data.payer === 'tenant' ? 'text-orange-600' : 'text-green-600'}`}>
                                                    {data.payer === 'tenant' ? 'Khách thuê trả' : (data.payer === 'landlord' ? 'Chủ trọ đài thọ' : data.payer_label)}
                                                </span>
                                            </div>
                                            
                                            {/* Giải thích rõ ràng cho khách thuê nếu họ phải trả tiền */}
                                            {data.payer === 'tenant' && (
                                                <div className="mt-1.5 sm:mt-2 p-2.5 bg-white border border-orange-200 rounded-lg text-[11px] sm:text-[12px] flex items-start gap-2 shadow-sm">
                                                    <i className="fa-solid fa-circle-info text-orange-500 mt-0.5 shrink-0"></i>
                                                    <span className="text-slate-600 leading-relaxed">
                                                        {data.invoice_id 
                                                            ? "Khoản phí này đã được cộng vào hóa đơn tiền phòng của bạn. Vui lòng kiểm tra mục Hóa đơn." 
                                                            : "Khoản phí này sẽ tự động được cộng vào hóa đơn tiền phòng của kỳ tiếp theo."}
                                                    </span>
                                                </div>
                                            )}
                                        </div>
