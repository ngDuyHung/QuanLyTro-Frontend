...
export default function ReportsPage() {
    ...
    // Mặc định: Từ đầu tháng đến hôm nay (Chuẩn định dạng YYYY-MM-DD cho backend)
    ...
    const [activeTab, setActiveTab] = useState("financial"); // 'financial', 'ledger', 'occupancy', 'debt'
    const [reportData, setReportData] = useState(null);
    const [isLoading, setIsLoading] = useState(true);
    const [isExporting, setIsExporting] = useState(false);

    // --- 3. FETCH DANH SÁCH KHU NHÀ CHO DROPDOWN ---
    ...
    // --- 4. FETCH DỮ LIỆU BÁO CÁO KHI FILTER HOẶC TAB THAY ĐỔI ---
    const fetchReportData = useCallback(async () => {
        setIsLoading(true);
        setReportData(null); // Clear data cũ tránh nháy giao diện
        try {
            const params = {
            ...
            switch (activeTab) {
                ...
                case "occupancy":
                    ...
                    break;
                case "debt":
                    // Báo cáo công nợ chỉ lấy thời điểm hiện tại, ko cần date
            ...
            // Error đã được api interceptor xử lý (hiện toast)
    ...
    // --- 5. HÀM XUẤT EXCEL CHO TAB SỔ QUỸ ---
    const handleExportExcel = async () => {
        try {
            ...
            // Xử lý tạo URL tải file từ Blob
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement("a");
            link.href = url;

            // Định dạng tên file: SoQuy_DDMMYYYY.xlsx
    ...
    const tabs = [
        ...
        { id: "debt", label: "Công nợ khách thuê", icon: "fa-file-invoice-dollar" },
    ];
    ...
    return (
        <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] p-4 md:p-6 lg:p-6 pt-6 flex flex-col h-full bg-slate-50">
            ...
            <div className="mb-5 flex flex-col lg:flex-row lg:items-end justify-between border-b border-slate-200 pb-4 gap-4">
                <div>
                    <h1 className="text-xl font-bold text-slate-800 mb-4">Báo cáo & Thống kê</h1>
                    <div className="flex overflow-x-auto no-scrollbar gap-2 lg:gap-6">
                        {tabs.map((tab) => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id)}
                                className={`pb-3 text-[13px] sm:text-[14px] font-medium transition-colors border-b-2 whitespace-nowrap px-1 flex items-center gap-2 ${activeTab === tab.id
                ...
                {/* NÚT XUẤT EXCEL (Chỉ hiện khi ở Tab Sổ Quỹ) */}
                {activeTab === "ledger" && (
                    <button
                        onClick={handleExportExcel}
                        disabled={isExporting || isLoading}
                        className="bg-white border border-green-500 text-green-600 px-4 py-2 rounded-lg text-[13px] font-medium hover:bg-green-50 flex items-center gap-2 shadow-sm transition-colors disabled:opacity-50"
                    >
                        {isExporting ? (
            ...
            {/* BỘ LỌC (FILTERS) */}
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 mb-6 flex flex-col md:flex-row items-end gap-4">
                ...
                {/* Lọc Ngày tháng (Ẩn đi nếu là Tab Công nợ) */}
                {activeTab !== "debt" && (
                    <>
                        <div className="w-full md:w-1/4">
                            ...
                            <input
                                ...
                                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-[13px] focus:outline-none focus:border-[#0e8b4d]"
                            />
                        </div>
                        <div className="w-full md:w-1/4">
                            <label className="block text-[12px] font-semibold text-slate-700 mb-1">
            ...
            <div className="flex-1 min-h-0 flex flex-col">
                {isLoading ? (
                    ...
                    <>
                        {/* Tạm thời hiển thị text để bạn hình dung, sau này thay bằng Component */}
                        {activeTab === "financial" && (
                            <FinancialReportTab data={reportData} />
                        )}

                        {activeTab === "ledger" && (
                            <LedgerReportTab data={reportData} />
                        )}

                        {activeTab === "occupancy" && (
                            <OccupancyReportTab data={reportData} />
                        )}

                        {activeTab === "debt" && (
                            <DebtReportTab data={reportData} />
                        )}
                    </>
                )}
            </div>
